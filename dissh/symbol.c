...
{
	char *p;
//	printf("segalloc: alloc memory %lx\n",size);
	p=malloc(size+16L);
	if(p==NULL) {
		printf("fatal: can't alloc memory!\n");
		exit(1);
	}
//	printf("segalloc: alloc memory %lx = %lx\n",size,(long)p);
	return p;
}
...
void init_symbol()
{
	...
	symbuf  =(SYMBOL *)  segalloc(0x20000L);
	namebuf =(char   *)  segalloc(0x20000L);
	namehash=(SYMBOL **) segalloc(0x20000L);
	adrshash=(SYMBOL **) segalloc(0x20000L);
