...
char    hexbuf[128];    char *hexp;     /* ヘキサダンプ */
char    mnebuf[32];     char *mnep;     /* ニーモニック */
char    op1buf[256];     char *op1p;     /* オペランド１ */
char    op2buf[256];     char *op2p;     /* オペランド２ */
char	rembuf[256];	char *remp;		/* remark       */
char    disbuf[256];    char *disp;     /* １行分のbuff */
char	symbol[256];
...
long    dd()                    /* ロングワード */
{
        ...
        dword=(long *)op;       /* ビッグエンディアンの機械では要注意！！！*/
        op+=4;
        return (*dword);
}
